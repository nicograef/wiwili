(function () {
  ...
  function LineGraph(data) {
      ...
      var countLines = groups(data, function (d) { return d.year; });
      var legendSpace = width / countLines.length; // spacing for the legend
      countLines.forEach(function (_a, i) {
          var year = _a[0], d = _a[1];
          // compute mean and title
          var counts = d.map(function (v) { return v.count; });
          var mean$1 = Math.floor(mean(counts) / 100) * 100;
          var title = year + ', ' + mean$1 + '/Tag';
          // add the line
          svg
              .append('path')
              .attr('class', 'line')
  ...
  function BarChart(data) {
      ...
      var y = linear$1()
          ...
          .domain([0, 18000]);
      // .domain([0, d3.max(data, d => d.count)])
      var countsPerYear = groups(data, function (d) { return d.year; });
      var legendSpace = width / countsPerYear.length;
      countsPerYear.forEach(function (_a, i) {
          var year = _a[0], d = _a[1];
  ...
  var parseTime = timeParse('%m-%d');
  var parseTimeWithYear = timeParse('%Y-%m-%d');
  csv$1('data/counts-by-year.csv').then(function (rawData) {
  ...
  function smooth(data, avgWidth) {
      ...
      data.forEach(function (d, i) {
          ...
          var localAverage = mean(localData.map(function (d) { return d.count; }));
